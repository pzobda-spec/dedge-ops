...
import { channelRange, channelMetrics, estimatedCallAverage, PHONE_BREAK_NOTE, monthStart, shiftMonth } from '@/lib/reporting/monthly'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const months = stats.months.map(month => ({ ...month, certified: certifiedPeriod(coverage, monthStart(month.key), monthStart(shiftMonth(month.key, 1))) }))
    return NextResponse.json({
      ...stats,
      months,
      totals: { ...stats.totals, ...estimatedCallAverage(months) },
