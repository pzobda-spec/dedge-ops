...
function ComplianceDetails({ title, data }: { title: string; data: ReturnType<typeof supportMetrics>['first_response_compliance'] }) {
  return <div className="rounded-lg border border-[#ded8e8] bg-white p-3">
    ...
    <p className="mt-2 text-xs text-[#696969]">Taux calculé sur {data.measured} / {data.population} tickets. Sans mesure : {data.without_measurement} · Non classés : {data.unclassified}{data.priorities.some(item => item.priority === 'P4' && !item.applicable && item.measured > 0) ? ' · P4 résolution : non applicable, volume affiché mais exclu du taux.' : ''}</p>
  </div>
}
  ...
  return <section aria-labelledby="monthly-report-title" className="space-y-5 border-b border-[#ded8e8] pb-8">
    ...
    {monthly && <>
      ...
      <section className={`${panel} space-y-4`} aria-labelledby="monthly-support-title">
        ...
        <div className="overflow-x-auto rounded-lg border border-[#ded8e8]"><table className="w-full min-w-[560px] text-sm"><thead className="bg-[#f7f3fc] text-xs text-[#696969]"><tr><th className="px-3 py-2 text-left">Indicateur</th><th className="px-3 py-2 text-right">{label(month)}</th><th className="px-3 py-2 text-right">{label(monthly.year_ago.month)}</th><th className="px-3 py-2 text-right">Écart</th></tr></thead><tbody>
          {[
            ['Tickets créés', number(monthly.support.opened), number(monthly.year_ago.support.opened), evolutionLabel(evolutionPct(monthly.support.opened, monthly.year_ago.support.opened))],
            ['Tickets clôturés', number(monthly.support.closed), number(monthly.year_ago.support.closed), evolutionLabel(evolutionPct(monthly.support.closed, monthly.year_ago.support.closed))],
            ['Clôturés / créés', number(monthly.support.closed_opened_pct, ' %'), number(monthly.year_ago.support.closed_opened_pct, ' %'), evolutionLabel(evolutionPoints(monthly.support.closed_opened_pct, monthly.year_ago.support.closed_opened_pct), ' points')],
            ['Conformité première réponse', number(monthly.support.first_response_compliance.rate_pct, ' %'), number(monthly.year_ago.support.first_response_compliance.rate_pct, ' %'), evolutionLabel(evolutionPoints(monthly.support.first_response_compliance.rate_pct, monthly.year_ago.support.first_response_compliance.rate_pct), ' points')],
            ['Conformité résolution', number(monthly.support.resolution_compliance.rate_pct, ' %'), number(monthly.year_ago.support.resolution_compliance.rate_pct, ' %'), evolutionLabel(evolutionPoints(monthly.support.resolution_compliance.rate_pct, monthly.year_ago.support.resolution_compliance.rate_pct), ' points')],
          ].map(([name, current, previous, change]) => <tr key={name} className="border-t border-[#eeeaf3]"><th className="px-3 py-2 text-left font-medium">{name}</th><td className="px-3 py-2 text-right tabular-nums">{current}</td><td className="px-3 py-2 text-right tabular-nums">{previous}</td><td className="px-3 py-2 text-right tabular-nums">{change}</td></tr>)}
        </tbody></table></div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <Kpi title="Tickets Phone" value={number(selectedChannel?.phone)} detail={selectedChannel ? `${number(selectedChannel.phone_share_pct, ' %')} du volume créé · canal Zoho brut.` : 'Chargement du détail par canal…'} />
          <Kpi title="Première réponse moyenne ouvrée (Zoho)" value={number(monthly.support.first_response_hours, ' h')} detail={`${monthly.support.first_response_sample} / ${monthly.support.opened} créations documentées. Durée ouvrée Zoho ; distincte du taux de conformité calendaire. ${evolutionLabel(evolutionPct(monthly.support.first_response_hours, monthly.year_ago.support.first_response_hours))}.`} />
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <Kpi title="Conformité première réponse" value={number(monthly.support.first_response_compliance.rate_pct, ' %')} detail={`Échantillon : ${monthly.support.first_response_compliance.measured} / ${monthly.support.first_response_compliance.population} tickets · Cible ${monthly.support.first_response_compliance.target_pct} % · ${complianceThresholdLine(monthly.support.first_response_compliance)} · ${evolutionLabel(evolutionPoints(monthly.support.first_response_compliance.rate_pct, monthly.year_ago.support.first_response_compliance.rate_pct), ' points')}`} />
          <Kpi title="Conformité résolution" value={number(monthly.support.resolution_compliance.rate_pct, ' %')} detail={`Échantillon : ${monthly.support.resolution_compliance.measured} / ${monthly.support.resolution_compliance.population} tickets · Cible ${monthly.support.resolution_compliance.target_pct} % · ${complianceThresholdLine(monthly.support.resolution_compliance)} · ${evolutionLabel(evolutionPoints(monthly.support.resolution_compliance.rate_pct, monthly.year_ago.support.resolution_compliance.rate_pct), ' points')}`} />
        </div>
        <div className="grid gap-3 lg:grid-cols-2">
        ...
        <p className={note}>Les durées de première réponse fournies par Zoho sont ouvrées. Elles ne servent pas au taux calendaire : sans horodatage fiable de première réponse, le ticket est sans mesure. Profil actif : <strong>crm</strong> · mapping approximatif des priorités (Urgent→P1, High→P2, Medium→P3, Low→P4), car aucune priorité P1–P4 n’est présente dans la source sur les six derniers mois · temps calendaires, sans horaires ouvrés ni jours fériés · P2–P4 première réponse et toute la résolution sont provisoires, reprises du profil groupe · paliers P1 : {p1PalierText} ; dates d’effet à renseigner, donc le palier actif est appliqué à toute la période et l’historique n’est pas remesuré au seuil passé · les délais résolution &gt;{monthly.support.resolution_max_days} jours sont non conformes au taux, même s’ils sont exclus de la moyenne.</p>
