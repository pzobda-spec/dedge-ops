...
export default function MonthlyReporting() {
  ...
  async function copy() {
    ...
    const lines = [
      ...
      ...(p ? [`Implémentation — lancements officiels selon la date de démarrage Zoho Projects : ${number(p.official_starts)} (CRM : ${p.official_starts_crm}, Dmbook : ${p.official_starts_dmbook}) ; transitions Non démarré → In Progress observées : ${number(p.status_transition_starts)} ; passés Live : ${p.went_live}.`,
    ...
    {monthly && <>
      ...
      <section className={`${panel} space-y-4`} aria-labelledby="monthly-implementation-title">
        ...
        {monthly.implementation.data ? <>
          <div className="grid gap-3 sm:grid-cols-2"><Kpi title="Lancements officiels" value={number(monthly.implementation.data.official_starts)} detail={`Date de démarrage Zoho Projects. CRM : ${monthly.implementation.data.official_starts_crm} · Dmbook : ${monthly.implementation.data.official_starts_dmbook}. Transition Non démarré → In Progress observée : ${monthly.implementation.data.status_transition_starts}.`} /><Kpi title="Projets passés Live" value={number(monthly.implementation.data.went_live)} detail={`${monthly.implementation.data.live_dated} selon Live date ; ${monthly.implementation.data.live_observed_without_date} transitions observées sans date métier. Chaque projet compte une fois par mois.`} /></div>
