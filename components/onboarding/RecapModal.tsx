...
${transcript}`
  ...
  async function copyContext() {
    ...
    setMessage(t('Contexte copié.'))
  }

  async function openGem() {
    setError(null)
    if (!gemUrl) {
      setError(t('Gem non configuré, voir Paramètres > Intégrations'))
      return
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="w-full max-w-2xl rounded-xl bg-white border border-slate-200 shadow-xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 p-5">
          <div>
            <h3 className="text-base font-semibold text-slate-900">{t('Récap du RDV d’implémentation')}</h3>
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <label className="block">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t('Hôtel')}</span>
          ...
          <label className="block">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t('Collez le transcript du RDV')}</span>
